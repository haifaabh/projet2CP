...
const isParentOrProprietaire = async (req, res, next) => {
...
router.post('/prendre_rdv/:id',isParentOrProprietaire, async (req, res) => {
  try {
...
router.delete('/annuler_rdv/:id',isParentOrProprietaire, async (req, res) => {
  try {
    ...
    res.status(200).send('Le rendez-vous a été annulé avec succès !');
  } catch (error) {
    console.error(error);
    res.status(500).send('Erreur dans le serveur');
  }
...
router.get('/parents',isParentOrProprietaire, async (req, res) => {    // afficher profil parent
    try {
      if (!req.session.user) {
  ...
  router.delete('/favoris/:id',isParentOrProprietaire,async(req,res)=> {     //Supprimer favoris
    try {
      if (!req.session.user) {
        return res.status(401).send('Accès non autorisé !!');
      }
      ...
      router.delete('/reservation/:id',isParentOrProprietaire, async (req, res) => {  // annuler reservation 
        try {
  ...
  router.put('/modifier_profil',isParentOrProprietaire, async (req, res) => {   //// modifier profil
    try {
      if (!req.session.user) {
        return res.status(401).send('Accès non autorisé !');
      }
...
router.post('/reserver/:id',isParentOrProprietaire, async (req, res) => {
  try {
    // Find the creche document with the matching name
    const creche = await Creche.findById(req.params.id);
    ...
    const enfant1 = await enfants.findOne({
      prenom: req.body.enfants.prenom,
      age: req.body.enfants.age,
      unite: req.body.enfants.unite
    });
    if (!enfant1) {
...
router.get('/afficher_reservations',isParentOrProprietaire, async (req, res) => {
  try {
    ...
    const reservations = await Reservation.find({ parent_id: parentId }).select('date nom_creche enfants etat -_id');
...
router.get('/afficher_Rdv', isParentOrProprietaire,async (req, res) => {
  try {
    // Get the parent ID from the session
    const parentId = req.session.user._id;
  
    const rdv = await Rdv.find({ parent_id: parentId }).select('date heure_debut heure_fin creche_id etat -_id');
    const creche = await Creche.findById(rdv.creche_id);
...
router.post('/creches/:id/commenter',isParentOrProprietaire,async (req,res)=>
{
  if (!req.session.user) 
  {
...
router.delete('/creches/:id/sup_commentaire/:index',isParentOrProprietaire, async (req, res) => {
  ...
  try {
    ...
    const comment = crecheDb.avis[index];
    if (comment.username !== username) {
      return res.status(401).send("Vous n'êtes pas autorisé à supprimer ce commentaire");
    }
...
router.get('/favoris',isParentOrProprietaire,async (req,res)=>
{
  if (!req.session.user) 
  {
    return res.status(401).send('Accès non authorisé !!');
...
router.post('/favoris/:id',isParentOrProprietaire,async(req,res)=>
{
  try
  {
    if (!req.session.user) 
...
router.get('/enfants',isParentOrProprietaire,async(req,res)=>
{
  if (!req.session.user) 
  {
    return res.status(401).send('Accès non authorisé !!');
...
router.post('/enfants/ajouter',isParentOrProprietaire,async(req,res)=>
{
  if (!req.session.user) 
  {
    return res.status(401).send('Accès non authorisé !!');
  }
  const newEnf = new Enfant({
    prenom : req.body.prenom,
    age : req.body.age,
    unite : req.body.unite
  });
  const enf = await Enfant.create(newEnf);
  const user = await User.findById(req.session.user);
...
router.delete('/enfants/:id',isParentOrProprietaire, async (req, res) => 
{
  ...
  await user.save();
  await Enfant.findByIdAndDelete(enfantId);
  res.status(200).send('Enfant supprimé avec succès !!');
});

router.put('/modifier_enfant/:enfantId', isParentOrProprietaire,async (req, res) => {
  try {
