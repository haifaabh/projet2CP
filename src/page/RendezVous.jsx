...
const RendezVous = ({ onPlusDinfoclick, onReserverClick, onAnnulerClick, id }) => {
  const initialValue = dayjs('2023-04-17'); // Valeur initiale pour la date sélectionnée
  const [isLoading, setIsLoading] = React.useState(false); // État de chargement
  const [statusMessage, setStatusMessage] = React.useState(''); // Message d'état
  const [value, setValue] = React.useState(dayjs('2022-04-17T15:30')); // Heure de fin du rendez-vous
  const [value1, setValue1] = React.useState(dayjs('2022-04-17T15:30')); // Heure de début du rendez-vous
  const [selectedDate, setSelectedDate] = useState(initialValue); // Date sélectionnée
  const [isSmallScreen, setIsSmallScreen] = useState(false); // État pour écran de petite taille
  ...
  const handleConfirmClick = async () => {
    const rdvData = {
      date: selectedDate.toISOString().substr(0, 10), // Date au format ISO
      heure_debut: value1.toISOString().substr(11, 5), // Heure de début au format ISO
      heure_fin: value.toISOString().substr(11, 5), // Heure de fin au format ISO
    };
    try {
  ...
  useEffect(() => {
    setIsSmallScreen(window.innerWidth < 670); // Vérifier si l'écran est de petite taille lors du chargement initial
    window.addEventListener('resize', () => setIsSmallScreen(window.innerWidth < 670)); // Mettre à jour l'état lorsque la taille de l'écran change
  }, []);
