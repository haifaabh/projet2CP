...
const Enfants = () => {
 ...
 const handleModification = (enfantId) =>{  
  ...
  function handleSave() {
    axios.put(`/api/parent/modifier_enfant/${selectedEnfantId}`, {
        prenom,
        age,
        unite,
      })
 ...
 const fetchEnfant= async (enfantId) => {
    try {
      const response = await axios.get(`/api/parent/afficher_enfant/${enfantId}`);
  ...
  const handleRemoveEnfant = (enfantId) => {
    axios
      .delete(`/api/parent/enfants/${enfantId}`)
      .then((res) => {
        if (res.status === 200) {
    ...
    <div className=' flex flex-row gap-4'>  
      ...
      <div className="w-full overflow-y-scroll hidden md:grid md:grid-cols-3 gap-4 justify-center md:pr-6">
        {enfants.map((enfant,index) => (
          <div key={index}>
            {showContent ? (
              <div className="relative w-full h-[270px] bg-white rounded-2xl p-2  mb-4 mr-8 mt-10 hover:bg-gray-50 shadow-lg">
        ...
        <Link to="/AjouterEnfant">
          <button className="bg-[#FBEBAC] text-[#094076] font-bold w-[190px] h-[40px] top-[100px] rounded-[12px] left-[500px] text-[18px] font-[Montserrat] absolute">
            Ajouter un enfant
          </button>
        </Link>
      </div>
