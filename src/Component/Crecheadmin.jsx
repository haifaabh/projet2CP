...
const Crecheadmin = () => {
  ...
  useEffect(() => {
    axios.get('/api/admin/afficher_toutes_creches')
      .then(res => setCreches(res.data))
      .catch(err => console.log(err));
  }, []);

  const handleRemoveCreche = (crecheId) => {
    axios.delete(`/api/admin/sup_creches/${crecheId}`)
      .then(res => {
        if (res.status === 200) {
    ...
    return (
        ...
        <div className="flex flex-wrap justify-center">
          ...
          {creches.map((creche, index) => (
            <div key={index} className="relative top-[10px] w-[360px] h-[310px] bg-white rounded-2xl p-2 m-4 hover:bg-gray-50 shadow-lg ">
