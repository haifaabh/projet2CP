...
const Attente = () => {
  ...
  useEffect(() => {
    const getCreches = async () => {
      const res = await axios.get('/api/AdminRoute/afficher_toutes_creches_attentes');
      setCreches(res.data);
    };
    getCreches();
  }, []);
  ...
  const handleAccepter = async (id) => {
    try {
      const result = await axios.post(`/api/AdminRoute/creche_attente/${id}`, { accepte: true });
    ...
    <div>
      ...
      <div className="flex flex-wrap justify-center">
        {creches.map((creche) => (
          <div
            key={creche._id}
            className="relative top-[150px] w-[330px] h-[270px] bg-white rounded-2xl p-2 m-4 hover:bg-gray-50 shadow-lg"
          >
            <Grid container spacing={1}>
              <Grid item xs={12}>
                ...
                <p className="border-b border-purple-400 px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent">
                  <span className="font-semibold">Tél :</span> {creche.tel}
                </p>
                <button
